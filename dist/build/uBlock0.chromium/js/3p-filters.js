...
const renderFilterLists = ( ) => {
    ...
    const initializeListEntry = (listDetails, listEntry) => {
        ...
        const filterContentElemt = qs$(listEntry, 'filter-content');
        dom.on(filterContentElemt, 'click', () => {
            const listKey = listEntry.dataset.key;
            const filterListObject = listsetDetails.available[listkey];
            const objectData = JSON.stringify(filterListObject, null, 2); // Format for readability
            const modalContent = document.createElement('div');
            modalContent.textContent = objectData;
        });
    };
